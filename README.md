...
`.flag-icon-xx` or  `.flag-icon-xxx` (where `xx` is the
[ISO 3166-1-alpha-2 code](http://www.iso.org/iso/country_names_and_code_elements) or `xxx` is the [ISO 3166-1-alpha-3 code](http://www.iso.org/iso/country_names_and_code_elements)
of a country) to an empty `<span>`. If you want to have a squared version flag
then add the class `flag-icon-squared` as well. Example:

```html
<span class="flag-icon flag-icon-gr [flag-icon-grc]"></span>
<span class="flag-icon flag-icon-gr [flag-icon-grc] flag-icon-squared"></span>
```
